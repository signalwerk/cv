import AvatarBlock from "./components/Avatar";
import ListBlock from "./components/List";
import SkillLevel from "./components/SkillLevel";
import SkillsBlock from "./components/SkillsBlock";

## Personal Details

<ListBlock
  data={`
* Name                   |     Stefan Huber
* Pronoun                |     he/him/his** (EN)** · er/ihm/sein** (DE)**
* Mail                   |     sh@signalwerk.ch
* Mobile                 |     +41 78 744 37 38
* Current residence      |     Zurich **· Switzerland**
* Year of birth          |     1981
* Nationality            |     Swiss
* Religion               |     Atheist **· and laughing about it**
* Marital status         |     Single
* Cyborg extensions      |     Glasses
* Social Media           |     [LinkedIn](https://www.linkedin.com/in/signalwerk) · [GitHub](https://github.com/signalwerk) · [Instagram](https://www.instagram.com/signalwerk/) · [Mastodon](https://mastodon.social/@signalwerk)
`}
/>

<hr />

<AvatarBlock />

## Professional Experience

<ListBlock
  data={`
* 2025 – now          | *Digital Lead* **· Stiftung Team Ensemble · Zurich – Switzerland**
* 2006 – now          | *Developer* **· Self Employed · Zurich – Switzerland**
* 2019 – 2025         | *Developer* **· Liip · Zurich – Switzerland**
* 2017 – 2021         | *Head of College Higher Education · Interaction Design* **· School of Design (SfGZ) · Zurich – Switzerland**
* 2007 – 2008         | *Assistant* **· Zurich University of the Arts (ZHdK) · Department of Design · Zurich – Switzerland**
* 2003 – 2005         | *Typesetter & Head of Information Technology* **· Team hp Schneider · Winterthur – Switzerland**
* 2002                | *Typesetter & Graphic Designer* **· ropress · Zurich – Switzerland**
* 2001 – 2002         | *Typesetter & Graphic Designer* **· gdz AG · Zurich – Switzerland**
`}
/>

## Degree

<ListBlock
  data={`
* 2023 – 2024         | *CAS Design Cultures* **· Certificate of Advanced Studies · Zurich University of the Arts (ZHdK) · Department of Design · Zurich – Switzerland**
* 2006 – 2008         | *CAS Type Design* **· Certificate of Advanced Studies · Zurich University of the Arts (ZHdK) · Department of Design · Zurich – Switzerland**
* 2002 – 2004         | *BP Typographic Designer* **· Federal Diploma of Higher Education · School of Design (SfGZ) · Zurich – Switzerland**
* 1997 – 2001         | *EFZ Polygraph* **· Federal Diploma of Vocational Education and Training · School of Design (SfGZ) · Zurich – Switzerland**
`}
/>

## Teaching & Trainings

<ListBlock
  data={`
* 2025                | *Pixelfonts like it’s 1982* **· Workshop · Vintage Computer Festival · Solothurn – Switzerland**
* 2025                | *Cyanotypie* **· Workshop · Hackerspace Odenwilusenz · Beringen – Switzerland**
* 2024                | *Pixelfonts like it’s 1982* **· Workshop · School of Design (SfGZ) · Zurich – Switzerland**
* 2023                | *Intaglio Printing* **· Workshop · Hackerspace Odenwilusenz · Beringen – Switzerland**
* 2022                | *Design Pixelfonts* **· Workshop · Hackerspace Odenwilusenz · Beringen – Switzerland**
* 2022                | *Lino Printing with and without AI* **· Workshop · Hackerspace Odenwilusenz · Beringen – Switzerland**
* 2022                | *Screen Printing and AI image generation* **· Workshop/Talk · SchatzWerk · Wergenstein – Switzerland**
* 2017 – 2023         | *Web Technology for Interaction Designers* **· Study Program · School of Design (SfGZ) · Zurich – Switzerland**
* 2014 – 2015         | *Web Technology for Designers* **· Study Program · School of Design (SfGZ) · Zurich – Switzerland**
* 2008                | *Technical Training for Designers* **· Study Program · University of Applied Sciences and Arts · Lucerne – Switzerland**
* 2007 – 2008         | *Technical Training for Typesetters* **· Study Program · School of Design (SfGZ) · Zurich – Switzerland**
* 2005 – 2006         | *Technical Training for Graphic Designers* **· Study Program · School of Design (SfGZ) · Zurich – Switzerland**
`}
/>

## Open Source

### Contributions

- Code contributions across 100+ projects on GitHub
- 100+ public repositories on GitHub
- Wikipedia Germany · Wikipedia English

### Community

- Regularly attended technology meetups
- Organized public meetups
- Attended many contribution days and hackathons
- Co-organizer of the first national technology camp in Switzerland (ZeTeCo)

## Skill Set

<SkillsBlock>

### Programming Languages

<SkillLevel
  data={`
* 95       |   JavaScript
* 80       |   TypeScript
* 40       |   Python
* 30       |   C & C++
* 35       |   Java
* 30       |   PHP
* 40       |   Shell/Bash Script
`}
/>

<ListBlock
  data={`
* Used in the past      | Ruby · XSLT
`}
/>

### AI Topics

<SkillLevel
  data={`
* 90       |   LLM & Prompt Engineering
* 80       |   RAG & Retrieval AI
* 70       |   Vector-Search (typesense)
* 70       |   Generative AI experiences (TTS, STT, Image, Video)
`}
/>

### Frameworks

<SkillLevel
  data={`
* 90       |   react.js
* 80       |   vue.js
* 70       |   Next.js
* 60       |   NestJS
* 70       |   express.js
`}
/>

<ListBlock
  data={`
* others                | processing.org · Paper.js
* Used in the past      | jQuery · backbone.js · ember.js
`}
/>

### Testing

<SkillLevel
  data={`
* 70       |   Jest
* 60       |   Mocha
* 60       |   Jasmine
`}
/>

### CSS Processors

<SkillLevel
  data={`
* 90       |   SCSS
* 90       |   PostCSS
* 70       |   LESS
`}
/>

### Build & package manager

<SkillLevel
  data={`
* 95       |   npm
* 95       |   vite
* 95       |   parcel
* 85       |   webpack
`}
/>
<ListBlock
  data={`
* Used in the past      | Grunt · gulp · Bower
`}
/>

### CI/CD

<SkillLevel
  data={`
* 85       |   GitHub Actions
* 80       |   GitLab CI/CD
* 80       |   Drone CI
* 70       |   Travis CI
`}
/>

### Database Proficiency

<SkillLevel
  data={`
* 80       |   Relational database (SQL)
* 70       |   Document-oriented database
`}
/>

### Good to know

<SkillLevel
  data={`
* 85       |   GIT/gitflow
* 90       |   RegEx
* 45       |   Linux/BSD Admin
* 75       |   Docker & Docker Compose
`}
/>

### Working with Designers

<SkillLevel
  data={`
* 60       |   Figma
* 70       |   Adobe Photoshop
* 90       |   Adobe InDesign
* 85       |   Adobe Illustrator
`}
/>
<ListBlock
  data={`
* Used in the past      | Adobe XD · Sketch
`}
/>

### Bar charts are not a good idea for skills in a CV

<SkillLevel
  data={`
* 90       |   Yes
`}
/>

</SkillsBlock>

## Programming Experience

### Paradigm & Platform

<ListBlock
  data={`
* 2017 – now         | Functional programming
* 2010 – now         | Hardware programming (Microcontroller)
* 2005 – now         | Object-oriented programming (OOP)
* 2000 – now         | Declarative programming
* 1998 – now         | Procedural programming
`}
/>

### Frameworks & Datafetching

<ListBlock
  data={`
* 2019 – 2025          | vue.js
* 2016 – now           | GraphQL
* 2015 – now           | react.js
* 2011 – 2014          | ember.js
* 2010 – now           | RESTful
`}
/>

### Database

<ListBlock
  data={`
* 2012 – now         | Document-oriented databases
* 2006 – 2010        | Relational Databases (SQL)
`}
/>

### Version Control

<ListBlock
  data={`
* 2010 – now         | GIT
* 2006 – 2010        | SVN
`}
/>

## Languages

<ListBlock
  data={`
* German     | Native
* English    | Fluent **· CEFR C2**
* French     | Intermediate **· CEFR B1**
* Spanish    | Basic **· CEFR A2**
`}
/>

## Language Certificates

<ListBlock
  data={`
* 2008     | Cambridge Certificate of Advanced English **· CAE**
* 2006     | Cambridge First Certificate in English **· FCE**
`}
/>

## Talks

<ListBlock
  data={`
* 2026                | *Working only for food and shelter – an experience in the 21st century* **· Lightning Talk · Electromagnetic Field 2026 · Ledbury – England**
* 2026                | *Building Slides* **· Lightning Talk · Libre Graphics Meeting · Nuremberg – Germany**
* 2024                | *Ikarus Splines* **· Lightning Talk · Libre Graphics Meeting · Rennes – France**
* 2024                | *How to preserve curves?* **· Day of Font · Zurich – Switzerland**
* 2019                | *Liberate books* **· Lightning Talk · Libre Graphics Meeting · Saarbrücken – Germany**
* 2019                | *State of the Font* **· School of Design (SfGZ) · Zurich – Switzerland**
* 2018                | *Git & GitHub* **· Meetup · Zurich – Switzerland**
* 2018                | *React.js* **· Meetup · Zurich – Switzerland**
* 2018                | *ramdajs* **· Meetup · Zurich – Switzerland**
* 2018                | *paper.js* **· Meetup · Zurich – Switzerland**
* 2018                | *Travis CI* **· Meetup · Zurich – Switzerland**
* 2018                | *New Fonts! – New language?* **· Literature Days · Zofingen – Switzerland**
* 2018                | *Generative Design shared in a URL* **· Lightning Talk · Libre Graphics Meeting · Sevilla – Spain**
* 2017                | *Abstraction in design – from hand to code* **· ZeTeCo · Schaffhausen – Switzerland**
* 2014                | *Responsive design – who adapts to whom?* **· Meeting Point Publishing · Zurich – Switzerland**
* 2011                | *Caminantes Grafico* **· Hyperwerk Institute of Postindustrial Design · Basel – Switzerland**
* 2010                | *New Fonts! Only why?* **· Day of Font · Zurich – Switzerland**
`}
/>

## Publications

<ListBlock
  data={`
* 2023                | *AI is part of the team at digital agencies* **· Persoenlich · Switzerland**
* 2020                | *Flexbox – aligning elements on the web* **· Publisher 3|2020 · Switzerland**
* 2020                | *Units of measurement on the web – relative or absolute* **· Publisher 2|2020 · Switzerland**
* 2020                | *Colours on the Web – distinguish and interpolate* **· Publisher 1|2020 · Switzerland**
* 2019                | *Spaces – Microtypography for the Web* **· Publisher 5|2019 · Switzerland**
* 2019                | *Design Systems for the Web* **· Publisher 4|2019 · Switzerland**
* 2019                | *Frontend-Frameworks – Websites today* **· Publisher 3|2019 · Switzerland**
* 2014                | *Format a lifetime* **· M0ire 2 · Chicago/Switzerland**
* 2013                | *How likely is the chance?* **· M0ire 1 · Chicago|Switzerland**
* 2012                | *Caminantes Grafico* **· Print Market · 64|2012 · Switzerland**
`}
/>

## Volunteer

<ListBlock
  data={`
* 2018 – now          | *Election worker* · elected by city council **· Zurich**
* 2013 – 2021         | *Auditor* · FabLab **· Zurich**
* 2013 – 2015         | *Board member* · Association for the tradition of the typographic arts **· Dresden – Germany**
* 2008 – 2013         | *Board assistant* · Graphics Forum Zurich **· Zurich – Switzerland**
* 1999 – 2008         | *Board member* · Youth Commission comedia **· Switzerland**
* 2019 – now          | *Member* · Digitale Gesellschaft **· Switzerland**
* 2013 – now          | *Member* · FabLab **· Zurich – Switzerland**
* 2013 – now          | *Member* · Association for the tradition of the typographic arts **· Dresden – Germany**
* 2022 – now          | *Member* · CH++ **· Switzerland**
`}
/>

## Social service <small>· mandatory in Switzerland</small>

<ListBlock
  data={`
* 2010 · 2 Month                | *Community worker* **· socio-therapeutic facility · Zurich – Switzerland**
* 2009 · 3 Month                | *Community worker* **· socio-therapeutic facility · Zurich – Switzerland**
* 2008 · 3 Month                | *Driver for disabled and elderly people* **· TIXI · Zurich – Switzerland**
* 2002 · 4 Month                | *Archivist* **· Swiss National Museum Zurich, Graphic Collection · Zurich – Switzerland**
`}
/>

## Internship

<ListBlock
  data={`
* 2009 · 6 Month         | *Typesetting with movable types* **· Offizin Haag Drugulin · Leipzig – Germany**
`}
/>

## Stays abroad

<ListBlock
  data={`
* 2025 · 1 Month     | *Sabbatical* **· Riga – Latvia**
* 2021 · 2 Month     | *Sabbatical* **· Various places – USA · Antwerp – Belgium**
* 2017 · 1 Month     | *Sabbatical* **· Ljubljana – Slovenia**
* 2012 · 4 Month     | *Sabbatical* **· New York – USA**
* 2011 · 3 Month     | *Sabbatical* **· Various places – USA**
* 2008 · 3 Month     | *English School* **· Sydney – Australia**
* 2006 · 3 Month     | *English School* **· London – UK**
* 2006 · 2 Month     | *Spanish School* **· Valencia – Spain**
`}
/>

## Sport

<ListBlock
  data={`
* 2010 – now    | 10 – 12 km swimming per week
* 1990 – 2000    | Judo
`}
/>

## Other Certificates

<ListBlock
  data={`
* Project Management      | TÜV SÜD Academy · HERMES Foundation **· 2025**
* Driving License         | Swiss driving licenses **· A1, B, D1 (3,5t;106), BE, D1E**
* Publishing              | PDFX-ready · Creator Certified Expert **· 2005**
`}
/>

## Personal Internet History

<ListBlock
  data={`
* ~1997           | Mail-Address
* 1999            | ebay Account
* 2004            | PayPal Account
* ~2005           | Podcast Listener
* 2008            | Editor-Status for [de.wikipedia.org](https://de.wikipedia.org/)
* 2011            | [Twitter Account](https://twitter.com/sgnlwrk) **· User 318 188 188**
* 2011            | [GitHub Account](https://github.com/signalwerk) **· User 992 878**
* 2012            | [Instagram Account](https://www.instagram.com/signalwerk/) **· User 218 821 516**
* 2012            | [Spotify Premium](https://open.spotify.com/user/signalwerk)
* 2016            | [TikTok Account](https://www.tiktok.com/@signalwerk) **(musical.ly)**
* 2019            | [Adoption of a character of «The Unicode Consortium»](https://unicode.org/consortium/adopted-characters.html#b2766) **· U+2766 ❦ FLORAL HEART**
* 2019            | Technical Preview: GitHub Actions
* 2020            | Technical Preview: GitHub Codespaces
* 2021            | Technical Preview: GitHub Copilot
* 2023            | [Threads Account](https://www.threads.net/@signalwerk) **· Previously blocked in Europe**
`}
/>

## Personal AI History

<ListBlock
  data={`
* 2023            | ChatGPT Pro
* 2022            | ChatGPT research preview
* 2022            | DALL·E 2
* 2021            | Technical Preview: GitHub Copilot
* 2021            | Tabnine Pro code completion
* 2016            | deepart.io
`}
/>

<br />
<br />
<hr />

<div className="small">

This page was made with React. The source code is available in my [GitHub repository](https://github.com/signalwerk/cv).  
This page respects your privacy and doesn't collect any behavior or user data.

</div>
